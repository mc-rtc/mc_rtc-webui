...
export class ArrayLabel extends Widget {
  ...
  draw() {
    ...
    ImGui.BeginTable(this.label("", "_table_data"), Math.max(this.data.length, this.labels.length), ImGui.ImGuiTableFlags.SizingStretchProp);
    ...
    for(let i = 0; i < this.data.length; ++i) {
      ...
      if(i == 0 && min === null) { min = ImGui.GetItemRectMin(); }
      if(i == this.data.length - 1) { max = ImGui.GetItemRectMax(); }
    }
    ImGui.EndTable();
