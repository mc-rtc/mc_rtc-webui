...
class TransformControls extends Object3D {
  constructor(camera, domElement) {
    ...
    defineProperty('active_mode', null);
    defineProperty('modes', ['translate', 'rotate']);
  ...
  pointerHover(pointer) {
    ...
    for (const mode of this.modes) {
      const intersect = intersectObjectWithRay(this._gizmo.picker[mode], _raycaster);
      if (intersect) {
        active_mode = mode;
...
class TransformControlsGizmo extends Object3D {
  constructor() {
    ...
    const matRed_rotate = matRed.clone();
    const matRed_scale = matRed.clone();

    const matGreen = gizmoMaterial.clone();
    matGreen.color.setHex(0x00ff00);

    const matGreen_rotate = matGreen.clone();
    const matGreen_scale = matGreen.clone();

    const matBlue = gizmoMaterial.clone();
    matBlue.color.setHex(0x0000ff);

    const matBlue_rotate = matBlue.clone();
    const matBlue_scale = matBlue.clone();
